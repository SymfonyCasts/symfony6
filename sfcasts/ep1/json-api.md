...
play button to life come. At the moment, none of these buttons do anything, but they
do look pretty.
...
Next, create a `public function` called, how about, `getSong()`. And then the
route: hit tab to auto-complete `Route` so that it adds the use statement on top.
...
our controllers will return a `Response` object, we can set that as the return type.
Make sure to auto-complete the one from Symfony's `HttpFoundation` component.

Inside the method, to start, let's `dd($id)`... *just* to see if everything is
working.

Let's do this: head over to `/api/songs/5` and... got it! *Another* new page.

Back in that controller, I'm going to paste in some song data, you can copy this
from the code block on this page. Eventually, this will come from the database.
Our goal is to return this as JSON.

So how *do* we return JSON in Symfony? By returning a new `JsonResponse` and passing
it the data.

I know... it's too easy! Refresh and... hello JSON! Now you *might* be thinking:

> Ryan, you dope! You've been telling us - repeatedly - that a controller must all
...
is nice because it automatically JSON encodes our data. It also sets the
...
... a `JsonResponse`! Oh and if you're building a serious API, Symfony has a
